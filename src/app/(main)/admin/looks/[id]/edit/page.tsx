...
// ── Normalise la catégorie d'un produit chargé depuis Supabase ──────────
// Si la catégorie n'est pas dans notre liste, on la met dans "autre"
...
export default function EditLookPage() {
  ...
  const fetchLookData = async () => {
    ...
    try {
      ...
      if (lookProducts && lookProducts.length > 0) {
        const mapped: ProductEntry[] = lookProducts.map((lp: any) => ({
          ...
          // ✅ FIX : normaliser la catégorie pour qu'elle corresponde à nos IDs
  ...
  // ✅ FIX : afficher TOUS les produits groupés, même ceux avec catégorie inconnue
  ...
  const getCloudinaryVideoUrl = (publicId: string) => {
    // On utilise la même base que getCloudinaryUrl mais pour les vidéos
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="max-w-5xl mx-auto px-4 py-8 space-y-6">
                ...
                <div className="relative rounded-xl overflow-hidden border-2 border-gray-200" style={{ aspectRatio: '3/4' }}>
                  ...
                  {!image.compressing && (
                    <div className="absolute bottom-2 left-2 right-2 bg-black/70 rounded-lg px-3 py-1.5 text-xs text-white flex justify-between">
                      ...
                      <span className="text-green-400 font-semibold">→ {image.compressedSize}</span>
                    </div>
                  )}
                  <button onClick={() => setImage(null)} className="absolute top-2 right-2 w-7 h-7 bg-black/60 text-white rounded-full flex items-center justify-center hover:bg-black transition-colors">
                    <X className="h-4 w-4" />
                  </button>
                </div>
              )}
            ...
            <div>
              ...
              {existingVideoId && !video && (
                <div className="relative rounded-xl overflow-hidden border-2 border-green-200" style={{ aspectRatio: '3/4' }}>
                  <video
                    key={existingVideoId}
                    src={getCloudinaryVideoUrl(existingVideoId)}
                ...
                <div className="relative rounded-xl overflow-hidden border-2 border-gray-200" style={{ aspectRatio: '3/4' }}>
                  ...
                  {video.compressing && (
                    <div className="absolute inset-0 bg-black/70 flex flex-col items-center justify-center gap-3 px-4">
                      ...
                      <div className="w-full bg-white/20 rounded-full h-2">
                        <div className="bg-purple-400 h-2 rounded-full transition-all" style={{ width: `${videoProgress}%` }} />
                      </div>
                  ...
                  {!video.compressing && (
                    <div className="absolute bottom-2 left-2 right-2 bg-black/70 rounded-lg px-3 py-1.5 text-xs text-white flex justify-between">
                      ...
                      <span className="text-green-400 font-semibold">→ {video.compressedSize}</span>
                    </div>
                  )}
                  <button onClick={() => { setVideo(null); setVideoProgress(0); }} className="absolute top-2 right-2 w-7 h-7 bg-black/60 text-white rounded-full flex items-center justify-center hover:bg-black transition-colors">
                    <X className="h-4 w-4" />
                  </button>
                </div>
              )}
        ...
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
          <div className="px-6 py-4 border-b border-gray-100">
            <h2 className="font-bold text-gray-900 flex items-center gap-2">
              <Tag className="h-5 w-5 text-purple-500" />Informations du look
            </h2>
          </div>
        ...
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
          ...
          <div className="p-6">
            ...
            {/* ✅ FIX : Afficher TOUS les produits groupés par catégorie */}
            {productsByCategory.length > 0 ? (
                            ...
                            {product.imageFile && !product.imageCompressing ? (
                              <div className="flex items-center gap-2">
                                ...
                                <button onClick={() => setProducts(prev => prev.map(p => p.tempId === product.tempId
                                  ? { ...p, imageFile: undefined, imagePreview: undefined, imageCompressed: undefined }
                                  : p))}
              ...
              /* ✅ Fallback : si aucun groupe ne matche, afficher tout à plat */
              <div className="space-y-4 mb-5">
                <p className="text-xs text-amber-600 bg-amber-50 px-3 py-2 rounded-lg border border-amber-200">
                  ⚠️ Certains produits ont des catégories non reconnues — ils sont affichés ici
                </p>
                {products.map(product => (
                  <div key={product.tempId} className="p-4 bg-gray-50 rounded-xl border border-gray-100">
                    <div className="flex items-center gap-2 mb-3">
                      <span className="text-xs text-gray-400 bg-gray-200 px-2 py-0.5 rounded-full">{product.category}</span>
                    </div>
                    <div className="grid grid-cols-12 gap-3">
                      <div className="col-span-4">
                        <input type="text" value={product.name} onChange={(e) => updateProduct(product.tempId, 'name', e.target.value)}
                      ...
                      <div className="col-span-1 flex items-center justify-center">
                        <button onClick={() => removeProduct(product.tempId)} className="p-2 text-red-400 hover:text-red-600 hover:bg-red-50 rounded-lg transition-colors">
                          <Trash2 className="h-4 w-4" />
                        </button>
                      </div>
                    </div>
            ...
            {products.length === 0 && (
              <p className="text-center text-xs text-gray-400 mt-3">Aucun produit associé à ce look</p>
            )}
          </div>
        </div>

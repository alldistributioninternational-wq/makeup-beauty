...
export default function NewProductPage() {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      setUploadProgress('Création...');
      const shadesData = hasShades && shades.length > 0 ? shades.filter(s => s.name.trim()).map(s => ({ id: s.tempId, name: s.name.trim(), hex: s.hex })) : null;
      const { error } = await supabase.from('products').insert({ name: name.trim(), brand: brand.trim(), price: parseFloat(price), category, description: description.trim() || null, cloudinary_id: cloudinaryId, shades: shadesData ? JSON.stringify(shadesData) : null });
  ...
  if (success) return (
    <div className="min-h-screen bg-gray-50 flex items-center justify-center">
      ...
      <h2 className="text-2xl font-bold text-gray-900 mb-2">Produit créé !</h2><p className="text-gray-500">Redirection...</p></div>
    </div>
  );
    ...
    <div className="min-h-screen bg-gray-50">
      <div className="bg-white border-b border-gray-200 sticky top-0 z-20">
        <div className="max-w-3xl mx-auto px-4 py-4 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div><h1 className="text-lg font-bold text-gray-900">Nouveau produit</h1><p className="text-xs text-gray-500">Remplissez les informations</p></div>
          </div>
          <button onClick={handleSave} disabled={saving || image?.compressing}
            ...
            {saving ? <><Loader2 className="h-4 w-4 animate-spin" />{uploadProgress || 'Sauvegarde...'}</> : <><CheckCircle className="h-4 w-4" />Ajouter</>}
          </button>
        </div>
      </div>
      ...
      <div className="max-w-3xl mx-auto px-4 py-8 space-y-6">
        ...
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
          <div className="px-6 py-4 border-b border-gray-100"><h2 className="font-bold text-gray-900 flex items-center gap-2"><Package className="h-5 w-5 text-purple-500" />Photo du produit</h2><p className="text-xs text-gray-400 mt-0.5">Compressée en WebP avant envoi</p></div>
          <div className="p-6 flex justify-center">
            <div className="w-64">
              <div onClick={() => !image && imageInputRef.current?.click()} className={`relative border-2 border-dashed rounded-2xl overflow-hidden ${!image ? 'cursor-pointer' : ''} ${errors.image ? 'border-red-400 bg-red-50' : 'border-gray-200 hover:border-purple-400 bg-gray-50'}`} style={{ aspectRatio: '1/1' }}>
                {image ? (<>
                  ...
                  <button onClick={(e) => { e.stopPropagation(); setImage(null); }} className="absolute top-2 right-2 w-7 h-7 bg-black/60 text-white rounded-full flex items-center justify-center"><X className="h-4 w-4" /></button>
                </>) : (<div className="absolute inset-0 flex flex-col items-center justify-center gap-2 text-gray-400"><Upload className="h-8 w-8" /><p className="text-sm">Cliquer pour uploader</p></div>)}
              </div>
        ...
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
          ...
          <div className="p-6 space-y-5">
            <div>
              ...
              <input type="text" value={name} onChange={e => setName(e.target.value)} placeholder="Ex: Velvet Matte Lipstick" className={`w-full px-4 py-3 rounded-xl border text-sm focus:outline-none focus:ring-2 focus:ring-purple-200 ${errors.name ? 'border-red-400' : 'border-gray-200 focus:border-purple-400'}`} />
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <input type="text" value={brand} onChange={e => setBrand(e.target.value)} placeholder="MAC, NARS..." className={`w-full px-4 py-3 rounded-xl border text-sm focus:outline-none focus:ring-2 focus:ring-purple-200 ${errors.brand ? 'border-red-400' : 'border-gray-200 focus:border-purple-400'}`} />
              ...
              <div>
                ...
                <input type="number" value={price} onChange={e => setPrice(e.target.value)} placeholder="29.90" min="0" step="0.01" className={`w-full px-4 py-3 rounded-xl border text-sm focus:outline-none focus:ring-2 focus:ring-purple-200 ${errors.price ? 'border-red-400' : 'border-gray-200 focus:border-purple-400'}`} />
        ...
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
          ...
          ) : <div className="px-6 py-4"><p className="text-sm text-gray-400 text-center">Pas de teintes (ex: mascara, fixateur...)</p></div>}
        </div>
        ...
        <div className="flex gap-3 pb-8">
          ...
          <button onClick={handleSave} disabled={saving || image?.compressing}
            ...
            {saving ? <><Loader2 className="h-5 w-5 animate-spin" />{uploadProgress}</> : <><CheckCircle className="h-5 w-5" />Ajouter le produit</>}
          </button>
        </div>
      </div>
    </div>
